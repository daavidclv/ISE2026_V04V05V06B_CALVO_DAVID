...
//      }
      ...
      else if (strcmp (var, "ctrl=Carrusel") == 0) { 
        // Nota: El navegador envía los espacios como '+'
        modo_leds = 1;
      }
      else if (strcmp (var, "ctrl=Alarma") == 0) {
        modo_leds = 2;
      }
...
uint32_t netCGI_Script (const char *env, char *buf, uint32_t buflen, uint32_t *pcgi) {
  ...
  switch (env[0]) {
    ...
    case 'b':
      ...
      if (env[2] == 'c') {
        // Mapear los 3 estados al desplegable <select>
...
//      }
    ...
    case 'g':
      ...
      switch (env[2]) {
        case '1':
          ...
          break;
        case '2':
          len = (uint32_t)sprintf (buf, &env[4], (adv*3.3f)/4096);
      ...
      // RTC Input from 'rtc.cgi'
        ...
        case '1':
          len = (uint32_t)sprintf (buf, &env[4], aShowDate);
					strcpy (lcd_text[0], aShowDate);
					osThreadFlagsSet (TID_Display, 0x02);
          break;
        case '2':
          len = (uint32_t)sprintf (buf, &env[4], aShowTime);
					strcpy (lcd_text[1], aShowTime);
					osThreadFlagsSet (TID_Display, 0x02);
          break;
      }
      break;		
    ...
    case 'x':
      // AD Input from 'ad.cgx'
		//adv = ADC_getVoltage(&adchandle , 10 );
      adv = AD_in (10);
