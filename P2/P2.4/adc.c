...
uint32_t ADC_getVoltage(ADC_HandleTypeDef *hadc, uint32_t Channel)
	{//Funcion modificada para devolver el codigo digital en lugar del voltaje
...
/*----------------------------------------------------------------------------
 *                       Cola de mensajes del volumen
 *---------------------------------------------------------------------------*/
  
osMessageQueueId_t mid_MsgQueueADC;                // message queue id

/*----------------------------------------------------------------------------
 *                                Thread ADC
 *---------------------------------------------------------------------------*/
osThreadId_t tid_Thread_adc;//id del thread

///////////Init thread///////////
int Init_Thread_adc (void) {
 
	//Creamos la cola de mensajes para la radio
	mid_MsgQueueADC = osMessageQueueNew(SIZE_MSGQUEUE_ADC , sizeof(SIZE_MSGQUEUE_ADC), NULL);
  if (mid_MsgQueueADC == NULL) {
    return (-1);
  }
	
  tid_Thread_adc = osThreadNew(Thread_adc, NULL, NULL);
  if (tid_Thread_adc== NULL) {
    return(-1);
  }
...
void Thread_adc (void *argument) {
  ...
  while (1) {
				...
				valor=ADC_getVoltage(&adchandle , 10 ); //get values from channel 10->ADC123_IN10
				newVoltage=(15/3.3)*valor;//me da el valor entero de valores enteros entre 0 y 15
			if(voltaje != newVoltage){
				localObject.Buf=newVoltage;//le paso el identificador 
				osMessageQueuePut(mid_MsgQueueADC, &localObject, 0U, 0U);//pongo el mensaje a la cola
				voltaje=newVoltage;
			}			 
			osDelay(250);
...
void Th_ADC_Test(void *argument); 
 
osThreadId_t tid_Th_ADC_Test;

int Init_Th_ADC_Test(void) {
	
  tid_Th_ADC_Test = osThreadNew(Th_ADC_Test, NULL, NULL);
  if (tid_Th_ADC_Test == NULL) {
    return(-1);
  }	
  return(0);
}
...
void Th_ADC_Test (void *argument) {
  ...
  while (1) {
	  ...
	  osMessageQueueGet(mid_MsgQueueADC, &localObject, 0U, 0U);
		TestADC=localObject.Buf;
		osDelay(1000);
